...
from my_datasets import (
    ...
    UCD_Dataset,
    UniPD_Dataset,
    get_cached_train_transform,
    get_cached_val_transform,
    get_subject_stratified_subset,
    get_train_transform,
    get_val_transform,
    read_cache_manifest,
)
...
def write_failure(dataset_name, trial_tag, phase, server_round, exc):
  ...
  print(
      f"[client:{dataset_name}] failure written to {path.resolve()}",
      file=sys.stderr,
      flush=True,
  )
  ...
  if torch.cuda.is_available():
    try:
      ...
      print(
          f"[client:{dataset_name}] gpu at failure "
...
def get_dataloaders(
    dataset_name, data_path, batch_size=4, image_size=512, cache_path=None
):
  if cache_path:
    ...
    print(
        f"[client:{dataset_name}] cached train={train_dir} val={val_dir} "
        f"bs={batch_size} (built {manifest['created_at']})"
    )

    train_ds = CachedOCTDataset(
        train_dir, transform=get_cached_train_transform()
    )
    ...
    print(
        f"[client:{dataset_name}] train={train_dir} val={val_dir} bs={batch_size}"
        f" crop={crop}"
    )
...
class Transport:
  ...
  def _request(
      self, method, path, body=None, meta=None, binary_response=False
  ):
    ...
    raise RuntimeError(
        f"{method} {path} failed after {HTTP_RETRIES} attempts: {last}"
    )
...
class OCTClient:
  ...
  def fit(self, rnd, config):
    ...
    # Gestione campionamento rotante per UCD in base a n_ms_subjects e n_ctrl_subjects
    n_ms = config.get("n_ms_subjects")
    n_ctrl = config.get("n_ctrl_subjects")

    if self.dataset_name == "ucd" and n_ms is not None and n_ctrl is not None:
      base_trainloader, _, _ = self._loaders(batch_size)
      full_train_ds = base_trainloader.dataset

      round_subset = get_subject_stratified_subset(
          full_train_ds,
          current_round=rnd,
          n_ms_subjs=int(n_ms),
          n_ctrl_subjs=int(n_ctrl),
      )
      round_sampler = build_downsampler(np.array(round_subset.targets))

      trainloader = torch.utils.data.DataLoader(
          round_subset,
          batch_size=batch_size,
          sampler=round_sampler,
          num_workers=4,
          pin_memory=True,
      )
      num_train_examples = len(round_sampler)
    else:
      trainloader, _, num_examples = self._loaders(batch_size)
      num_train_examples = num_examples["trainset"]

    # --- Verifica richiesta da UCD: loop di controllo sul numero di campioni serviti dal DataLoader ---
    actual_sampled = sum(1 for _ in trainloader.sampler)
    print(
        f"[client:{self.dataset_name}] DataLoader verification: "
        f"{actual_sampled} samples yielded by sampler (expected: {num_train_examples})"
    )
    assert actual_sampled == num_train_examples, (
        f"Mismatch! Dataloader sampler yielded {actual_sampled} images, "
        f"but expected {num_train_examples}"
    )
    # ---------------------------------------------------------------------------------------------------
    ...
    return params, num_train_examples, metrics
  ...
  def run(self):
    while not self.stop_event.wait(HEARTBEAT_INTERVAL_S):
      try:
        ...
        print(
            f"[client:{self.dataset_name}] heartbeat failed: {e}",
            file=sys.stderr,
            flush=True,
        )
...
def main():
  ap = argparse.ArgumentParser(
      description="Federated client for the MS classifier"
  )
  ap.add_argument(
      "--site",
      required=True,
  ...
  ap.add_argument(
      "--data-path", required=True, help="directory holding train/ and val/"
  )
  ap.add_argument(
      "--server", required=True, help="e.g. http://140.226.4.75:9092"
  )
  ap.add_argument(
      "--token",
      default=os.environ.get("FL_TOKEN", ""),
  ...
  ap.add_argument(
      ...
      default=None,
      help=(
          "directory of .pt tensors from cache_images.py; "
  ...
  print(
      f"[client:{dataset_name}] torch={torch.__version__} "
  ...
  try:
    ...
    print(
        paint(
            f"[client:{dataset_name}] joined as {display} at {now_hms()} "
            f"({info.get('num_rounds')} rounds)",
            GREEN,
        ),
        flush=True,
    )
    ...
    if torch.cuda.is_available():
      ...
      if args.gpu_index >= n_gpus:
        raise ValueError(
            f"gpu_index={args.gpu_index} but only {n_gpus} CUDA device(s)"
            " visible"
        )
    ...
    while True:
      ...
      if phase == "done":
        print(
            paint(
                f"[client:{dataset_name}] run finished at {now_hms()}", GREEN
            ),
            flush=True,
        )
        break
      ...
      if phase == "aborted":
        print(
            paint(
                f"[client:{dataset_name}] server aborted the run at {now_hms()}",
                RED,
            ),
            file=sys.stderr,
            flush=True,
      ...
      if st["submitted"]:
        ...
        if waiting_on != key:
          waiting_on = key
          print(
              f"[client:{dataset_name}] submitted round {rnd} {phase} "
              f"at {now_hms()} — waiting for other sites",
              flush=True,
          )
    ...
    try:
      ...
      print(
          f"[client:{dataset_name}] could not notify server: {e}",
          file=sys.stderr,
          flush=True,
      )
    if hb:
